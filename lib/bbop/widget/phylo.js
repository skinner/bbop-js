...
renderer.prototype.show_pgraph = function(pgraph) {
    ...
    var node_id_list = jQuery.map( tree_renderer.leaves(),
    ...
    var leaf_id_list = jQuery.map( tree_renderer.leaves(),
                                   function(x) { return x.id; } );
...
renderer.prototype.setup_drag_handle = function() {
    ...
    if (this.config.show_headers) {
        ...
        scale_drag_handle.style.cssText = [
            ...
            "width: " + Math.min(20, this.config.header_height) + "px",
            "background: #5f5",
    ...
    jQuery(this.scale_drag_elem).draggable({
        ...
        start: function(event, ui) {
            self.tree_renderer.width_update_start();
        },
        ...
        stop: function(event, ui) {
            self.tree_renderer.width_update_end();
        }
    });
};
